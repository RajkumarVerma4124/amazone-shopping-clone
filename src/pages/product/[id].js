...
function Details({ product, productsFurniture }) {
    ...
    return (
        <>
            ...
            <Header productsFurniture={productsFurniture} />
                ...
                <div className="max-w-screen-2xl mx-auto">
                    ...
                    <div className="grid grid-flow-row-dense md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        {productsFurniture && productsFurniture.slice(0, 4).map((product) => (
                            <ProductFurniture productsFurniture={productsFurniture} key={product.id} id={product.id} name={product.name} shipping={product.shipping} price={product.price} company={product.company} description={product.description} category={product.category} image={product.image} colors={product.colors} />
                        ))}
                    </div>
                </div>
                <Footer />
...
export const getStaticPaths = async () => {
    ...
    const paths = productsFurniture.map(product => {
        return {
...
export const getStaticProps = async (context) => {
    ...
    return {
        props: { product, productsFurniture }
    }
}